...
use crate::attributes::{StringPoolBuilder, Strings};
...
fn compile_node(
    ...
    for (i, attr) in node.attributes().enumerate() {
        ...
        } else if attr.name() == "platformBuildVersionCode" || attr.name() == "platformBuildVersionName" {
            ResValue { size: 8, res0: 0, data_type: ResValueType::IntDec as u8, data: attr.value().parse()?, }
        } else {
            ResValue { size: 8, res0: 0, data_type: ResValueType::String as u8, data: strings.id(attr.value()) as u32, }
        };
    ...
    let namespace = node.tag_name().namespace().map(|ns| strings.id(ns)).unwrap_or(-1);
